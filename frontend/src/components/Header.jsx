...
export default function Header({ cartCount = 0, isAdmin = false, isLoggedIn = false, searchQuery, setSearchQuery, onLogout }) {
    ...
    <header className="sticky top-0 z-40 border-b border-ticket-border/70 bg-ticket-bg/80 backdrop-blur-xl">
      <div className="container-page flex items-center gap-4 py-4">
        ...
        <div className="hidden items-center gap-3 sm:flex">
          <Button to={isLoggedIn ? (isAdmin ? '/admin/eventos' : '/mis-entradas') : '/login'}>
            {isLoggedIn ? (isAdmin ? 'Panel Admin' : 'Mis Entradas') : 'Ingresar'}
          </Button>
