...
export default function Home() {
  ...
  const filtered = category === 'Todos' ? events : events.filter((event) => event.category === category)
      ...
      <main className="container-page py-8">
        <section className="relative overflow-hidden rounded-[2rem] border border-ticket-purple2/30 bg-ticket-card shadow-glow">
          ...
          <div className="relative max-w-3xl p-8 sm:p-12 lg:p-16">
            <div className="flex flex-wrap gap-3"><Badge>Destacado</Badge><Badge tone="red">Últimas entradas</Badge></div>
        ...
        <section className="mt-10">
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filtered.length ? filtered.map((event) => <EventCard key={event.id} event={event} />) : <div className="col-span-full rounded-3xl border border-ticket-border bg-ticket-card p-10 text-center text-gray-400">No hay eventos en esta categoría por ahora.</div>}
          </div>
        </section>
      </main>
    </div>
