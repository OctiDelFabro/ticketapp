...
function Field({ label, error, help, children, className='' }) { return <label className={`block min-w-0 ${className}`}><span className="mb-2 block text-sm font-bold text-gray-300">{label}</span>{children}{help && <span className="mt-1 block whitespace-normal break-words text-xs leading-5 text-gray-500">{help}</span>}{error && <span className="mt-1 block text-sm text-red-400">{error}</span>}</label> }
